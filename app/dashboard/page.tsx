...
const rankIcon = (i: number) =>
  i === 0 ? (
    <Trophy style={{ height: 20, width: 20, color: "#f59e0b" }} />
  ) : i === 1 ? (
    <Medal style={{ height: 20, width: 20, color: "#9ca3af" }} />
  ) : i === 2 ? (
    <Award style={{ height: 20, width: 20, color: "#b45309" }} />
  ) : (
...
export default function AdminPage() {
  ...
  // Complete email-link sign-in if returning with oobCode in URL
  useEffect(() => {
    ...
    (async () => {
      try {
        // Prefer saved email, else prompt the user
        ...
        setLinkSentTo(null);
        toast.success("Signed in with magic link", { description: result.user.email || "" });
      } catch (err: any) {
        toast.error("Magic link sign-in failed", { description: err?.message });
      }
    })();
  }, []);
  ...
  const [createKolState, setCreateKolState] = useState<
    "idle" | "success" | "error"
  >("idle");

  // ---- Custom Modal (Manage Admins)
  const [adminEmail, setAdminEmail] = useState("");
  const [roleBusy, setRoleBusy] = useState<"grant" | "revoke" | null>(null);
  const [showAdminModal, setShowAdminModal] = useState(false);

  // server-verified permissions
  const canCreateKOL = who.isAdmin;
  const canDeleteKOL = who.isAdmin;
  ...
  const sendMagicLink = async () => {
    ...
    try {
      setSendingLink(true);
      // Configure deep link back to this page
  ...
  const verifyAccess = async () => {
    if (!code.trim())
      return toast.error("Code required", { description: "Enter your team ID" });
    try {
      setChecking(true);
      if (!auth.currentUser)
      ...
      if (!snap.exists()) {
        ...
        return toast.error("Invalid code", {
          description: "No access record found",
        });
        }
  ...
  const createKOL = async (): Promise<boolean> => {
    if (!auth.currentUser) {
      toast.error("Sign-in required");
      return false;
    }
    if (!canCreateKOL) {
      toast.error("Insufficient role", {
  ...
  const kolPerChainTotals = useMemo(() => {
    ...
    for (const d of mintDocs) {
      ...
      Object.entries(pc).forEach(([chain, bucket]) => {
        ...
        acc.compute[chain] =
          (acc.compute[chain] || 0) + n((bucket as any).compute);
      });
    }
    return acc;
  }, [mintDocs]);
  ...
  const timeFiltered = useMemo(() => {
    ...
    const filtered = deduped.filter((e) => {
      ...
      return rangeMode === "MONTHS_IN_YEAR"
    ...
    if (rangeMode === "MONTHS_IN_YEAR") {
      const byMonth = Array.from({ length: 12 }, (_, i) => ({
        label: new Date(selectedYear, i, 1).toLocaleString(undefined, {
          month: "short",
        }),
  ...
  // -----------------------------
  // Sign-in screen (Google + Email-Link)
  // -----------------------------
  if (!user) {
    return (
      <div
      ...
      >
        <div style={{ width: "100%", maxWidth: 460 }}>
          <Card
            ...
            right={<ShieldCheck size={18} color="#2563eb" />}
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: "#111827",
                  color: "#fff",
                  cursor: sendingLink ? "wait" : "pointer",
                  fontWeight: 700,
                }}
              >
              ...
              {linkSentTo && (
                <p style={{ color: "#059669", fontSize: 13 }}>
                  Magic link sent to <strong>{linkSentTo}</strong>. Open it on this
                  device to finish sign-in.
                </p>
              )}
            </div>
          </Card>
  ...
  // ---------- Metrics for render ----------
  const totalMints =
    kolTotals.seed + kolTotals.tree + kolTotals.solar + kolTotals.compute;
  const totalValue =
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#e6f0fa", padding: 16 }}>
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <span
              style={{
                fontSize: 12,
                color: who.isAdmin ? "#059669" : "#b91c1c",
                fontWeight: 700,
              }}
            >
              {who.isAdmin ? "Admin ✓" : "Not Admin"}
            </span>

            {/* Custom Modal Trigger for Super Admin */}
            {who.isSuperAdmin && (
              <button
        ...
        <Card
          ...
          right={
            <span
              style={{
                fontSize: 12,
                color: canCreateKOL ? "#059669" : "#b91c1c",
                fontWeight: 700,
              }}
            >
              {canCreateKOL ? "WRITE ENABLED" : "WRITE LOCKED"}
            </span>
          }
        >
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <input
                ...
                onChange={(e) =>
                  setKolForm((s) => ({ ...s, walletAddress: e.target.value }))
                }
                placeholder="0x…"
                style={{ padding: "0.75rem", border: "1px solid #d1d5db", borderRadius: 8 }}
              />
          ...
          <div style={{ display: "flex", justifyContent: "center", marginTop: 16 }}>
            <button
              ...
              style={{
                padding: "1rem 1.5rem",
                background: createKolState === "error" ? "#dc2626" : "#16a34a",
                color: "#fff",
                borderRadius: 14,
                border: 0,
                fontWeight: 800,
          ...
          right={
            canDeleteKOL ? (
              <span style={{ fontSize: 12, color: "#b45309", fontWeight: 700 }}>
                Admin can delete KOLs
              </span>
            ) : (
              <span style={{ fontSize: 12, color: "#6b7280" }}>View only</span>
            )
          }
        >
        ...
        >
          <Stat
            label="Approx. Recorded Events (last 500 per KOL)"
            value={totalEventsApprox}
          />
          <Stat label="Total NFTs Minted (KOL-linked)" value={totalMints} />
          <Stat
            label="Total Value Generated (KOL-linked)"
              ...
              <tbody>
                {leaderboard.map((row, i) => (
                  ...
                  >
                    ...
                    <td style={{ padding: 12, textAlign: "center", fontWeight: 800, color: "#059669" }}>
        ...
        >
          ...
          >
            ...
            <div style={{ padding: "1rem", display: "grid", gap: 10 }}>
              <label style={{ fontSize: 14, color: "#374151" }}>User Email</label>
              <input
                type="email"
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  border: "1px solid #e5e7eb",
                  background: "#fff",
                  cursor: !adminEmail || roleBusy ? "not-allowed" : "pointer",
                  fontWeight: 600,
                }}
              >
